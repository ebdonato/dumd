...
`go install` builds on Windows still get the `.exe` file icon shown in Explorer, version info, per-monitor DPI, and a non-blocking terminal — the icon/manifest/version resources are compiled into the binary from a committed resource object (see [`internal/winres`](#project-structure)), and DPI/console handling is set up at startup in `console_windows.go`.
...
> **Important:** Prefer `wails build` / `wails dev`. Wails orchestrates the Go compile, Vite frontend build, and asset embedding. Plain `go build` / `go install` also work because `frontend/dist/` is committed — Windows icon/manifest/version info come along too, from the committed resource object in `internal/winres/` — but they skip the Windows GUI subsystem flag and `-s -w` stripping that `wails build` applies.
...
├── resources_windows.go     # Links the committed Windows resource object (go build/go install only)
├── wails.json               # Wails project config (window size, build scripts)
├── go.mod / go.sum          # Go module dependencies
│
├── internal/winres/         # Committed Windows resource objects (icon, manifest, version info)
│   └── dumd_windows_*.syso  # Generated by tools/resgen — see below
│
├── tools/resgen/            # Generator for internal/winres/*.syso, mirrors `wails build`'s packaging
│
...
│   └── windows/             # Windows manifest, icon, installer configs — source for tools/resgen
│
